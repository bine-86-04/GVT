var app = (function() {
	...
	function setLightAngle() {

		setAngle += Math.PI / 40;
		var angle = [20, 60, 90, 120, 150, 200];

		var lightX;
		var lightY;
		var lightZ;

		//Loop for translate spheres
		for (var i = 1; i <= illumination.light.length; i++) {

			switch(i) {

				case 1:
				var angleX = setAngle + angle[i-1] * angle[1];
	            lightX = 1.5 * Math.sin(angleX);
				lightY = 0.5;
				lightZ = 1.5 * Math.cos(angleX);

			    illumination.light[0].position[0]  = lightX;
			    illumination.light[0].position[1]  = lightY;
			    illumination.light[0].position[2]  = lightZ;
				break;

                case 2:
                var angleX = setAngle + angle[i-1] * angle[1] + 210;
                lightX = 1.5 * Math.sin(angleX);
            	lightY = 0.5;
            	lightZ = 1.5 * Math.cos(angleX);

                illumination.light[1].position[0]  = lightX;
                illumination.light[1].position[1]  = lightY;
                illumination.light[1].position[2]  = lightZ;
